...
public class FirstServiceImpl implements FirstService {
    ...
    public void indexmng(Model model) throws Exception {
        ...
        model.addAttribute("highChartsDemoCustom", null);
        model.addAttribute("yjzzyrs", null);
        model.addAttribute("yjzrs", null);
    }
    ...
    public Model first(Model model, ActiveUser activeUser) throws Exception {
        ...
        if (roles != null && roles.size() > 0) {
            ...
            if (pollpatient.equals("1"))
                if (roles.contains("1001") || activeUser.getUsrno().equals("admin")) isdisplay_polling = "1";
    ...
    public Model firstHis(Model model, ActiveUser activeUser, HspemginfCustom hspemginfCustom, String type) throws Exception {
        ...
        if (roles != null && roles.size() > 0) {
            ...
            if (null != hspemginfCustom && StringUtils.isNotNullAndEmptyByTrim(hspemginfCustom.getEmgSeq())) {
                ...
                //不为空且大于0，说明有留抢数据
                if (StringUtils.isNotNullAndZero(sqlList)) type = "qjsCase";
                else type = "qjsCaseMz";
                if ("qjsCase".equalsIgnoreCase(type)) hisTitle = hspemginfCustom.getCstNam() + "-留抢病历记录";
                else if ("qjsCaseMz".equalsIgnoreCase(type)) hisTitle = hspemginfCustom.getCstNam() + "-门诊病历记录";
                ...
                //没有传递参数，则是门诊
                if (StringUtils.isNotNullAndEmptyByTrim(type)) type = "qjsCase";
                else type = "qjsCaseMz";
                //时间
                ...
                //三墩院区预检号标识 SD
                //灵隐院区预检号标识 LY
                if ("SD".equals(HSPLP)) hspemginfCustom.setEmgSeq("SD" + emgseq);
                else
