package activetech.hospital.pojo.domain;

import java.util.Date;

public class HspEmgInf {
    private String emgSeq;

    private String vstCad;

    private String idNbr;

    private String cstNam;

    private String cstSexCod;

    private Date bthDat;

    private String cstAge;

    private String pheNbr;

    private String cstAdr;

    private String grnChl;

    private String grnChlRsn;

    private Date emgDat;

    private String arvChlCod;

    private String cmpPsnCod;

    private String emgAreCod;

    private String emgDepCod;

    private String preDgnCod;

    private String preUsrNbr;

    private String preUsrNam;

    private String modGdnCod;

    private String modUsrNbr;

    private String modUsrNam;

    private String cstDspCod;

    private String emgFkCod;

    private String paiLvlCod;

    private String falAssCod;

    private String mnsSitCod;

    private Date mnsLstDat;

    private String algTypCod;

    private String symSitDes;

    private String savMngCod;

    private String sqlSeq;

    private Short oxyNbr;

    private String senStuCod;

    private String algHon;

    private String sqlDctNbr;

    private String pastHisCod;

    private String cstAgeCod;

    private String refusedSurvey;

    private String jbzdDes;

    private String cspgFlg;

    private String cspgCod;

    private String hxknFlg;

    private String hxknCod;

    private String hxknSubCod;

    private String xtCod;

    private String xtSubCod;

    private String xmCod;

    private String xmSubCod;

    private String xtFlg;

    private String xmFlg;

    private String swChl;

    private String emgJob;

    private String maritalStatus;

    private String emgBed;

    private String nowHis;

    private String pastHis;

    private String caseHis;

    private String isFetation;

    private String ksfz;

    private String patid;

    private String ryzdXy;

    private String cyzdZy;

    private String cyzdXy;

    private String observationCode;

    private String deathDiagnosis;

    private String specialOther;

    private Date docDat;

    private String patientTyp;

    private String pregnantYdcxCod;

    private String pregnantGsplCod;

    private Long jzxh;

    private String jzys;

    private String ysxm;

    private String ksdm;

    private String checkDoctor;

    private String checkNurse;

    private Date checkDocDat;

    private Date checkNurseDat;

    private String checkDocStatus;

    private String checkNurStatus;

    private String mpi;

    private String ghxh;

    private String rfidcode;

    private String vaccineHis;

    private String xfztFlg;

    private String qtsjFlg;

    private String algTypDes;

    private String bedLgs;

    private String firstsqlseq;

    private String cleanhzMemo;

    private String qtsjSeq;

    private Date mysj;

    private Date mqsj;

    private String cardType;

    private String triageNum;

    private String nationality;

    private String nation;

    private String gzdw;

    private String ghid;

    private String frCod;

    private Date frDat;

    private String firstemgseq;

    private String infectedareaNam;

    private String abnScoNme;

    private String abnDriver;

    private String abnDoctor;

    private String adtA01;

    private String glsType;

    private String glsNum;

    private String cdxx;

    private String khlx;

    private String psryqt;

    private String ghbz;

    private String glsCod;

    private String labelPrintFlg;

    private String xtlcflg;

    private String czlcflg;

    public String getEmgSeq() {
        return emgSeq;
    }

    public void setEmgSeq(String emgSeq) {
        this.emgSeq = emgSeq == null ? null : emgSeq.trim();
    }

    public String getVstCad() {
        return vstCad;
    }

    public void setVstCad(String vstCad) {
        this.vstCad = vstCad == null ? null : vstCad.trim();
    }

    public String getIdNbr() {
        return idNbr;
    }

    public void setIdNbr(String idNbr) {
        this.idNbr = idNbr == null ? null : idNbr.trim();
    }

    public String getCstNam() {
        return cstNam;
    }

    public void setCstNam(String cstNam) {
        this.cstNam = cstNam == null ? null : cstNam.trim();
    }

    public String getCstSexCod() {
        return cstSexCod;
    }

    public void setCstSexCod(String cstSexCod) {
        this.cstSexCod = cstSexCod == null ? null : cstSexCod.trim();
    }

    public Date getBthDat() {
        return bthDat;
    }

    public void setBthDat(Date bthDat) {
        this.bthDat = bthDat;
    }

    public String getCstAge() {
        return cstAge;
    }

    public void setCstAge(String cstAge) {
        this.cstAge = cstAge == null ? null : cstAge.trim();
    }

    public String getPheNbr() {
        return pheNbr;
    }

    public void setPheNbr(String pheNbr) {
        this.pheNbr = pheNbr == null ? null : pheNbr.trim();
    }

    public String getCstAdr() {
        return cstAdr;
    }

    public void setCstAdr(String cstAdr) {
        this.cstAdr = cstAdr == null ? null : cstAdr.trim();
    }

    public String getGrnChl() {
        return grnChl;
    }

    public void setGrnChl(String grnChl) {
        this.grnChl = grnChl == null ? null : grnChl.trim();
    }

    public String getGrnChlRsn() {
        return grnChlRsn;
    }

    public void setGrnChlRsn(String grnChlRsn) {
        this.grnChlRsn = grnChlRsn == null ? null : grnChlRsn.trim();
    }

    public Date getEmgDat() {
        return emgDat;
    }

    public void setEmgDat(Date emgDat) {
        this.emgDat = emgDat;
    }

    public String getArvChlCod() {
        return arvChlCod;
    }

    public void setArvChlCod(String arvChlCod) {
        this.arvChlCod = arvChlCod == null ? null : arvChlCod.trim();
    }

    public String getCmpPsnCod() {
        return cmpPsnCod;
    }

    public void setCmpPsnCod(String cmpPsnCod) {
        this.cmpPsnCod = cmpPsnCod == null ? null : cmpPsnCod.trim();
    }

    public String getEmgAreCod() {
        return emgAreCod;
    }

    public void setEmgAreCod(String emgAreCod) {
        this.emgAreCod = emgAreCod == null ? null : emgAreCod.trim();
    }

    public String getEmgDepCod() {
        return emgDepCod;
    }

    public void setEmgDepCod(String emgDepCod) {
        this.emgDepCod = emgDepCod == null ? null : emgDepCod.trim();
    }

    public String getPreDgnCod() {
        return preDgnCod;
    }

    public void setPreDgnCod(String preDgnCod) {
        this.preDgnCod = preDgnCod == null ? null : preDgnCod.trim();
    }

    public String getPreUsrNbr() {
        return preUsrNbr;
    }

    public void setPreUsrNbr(String preUsrNbr) {
        this.preUsrNbr = preUsrNbr == null ? null : preUsrNbr.trim();
    }

    public String getPreUsrNam() {
        return preUsrNam;
    }

    public void setPreUsrNam(String preUsrNam) {
        this.preUsrNam = preUsrNam == null ? null : preUsrNam.trim();
    }

    public String getModGdnCod() {
        return modGdnCod;
    }

    public void setModGdnCod(String modGdnCod) {
        this.modGdnCod = modGdnCod == null ? null : modGdnCod.trim();
    }

    public String getModUsrNbr() {
        return modUsrNbr;
    }

    public void setModUsrNbr(String modUsrNbr) {
        this.modUsrNbr = modUsrNbr == null ? null : modUsrNbr.trim();
    }

    public String getModUsrNam() {
        return modUsrNam;
    }

    public void setModUsrNam(String modUsrNam) {
        this.modUsrNam = modUsrNam == null ? null : modUsrNam.trim();
    }

    public String getCstDspCod() {
        return cstDspCod;
    }

    public void setCstDspCod(String cstDspCod) {
        this.cstDspCod = cstDspCod == null ? null : cstDspCod.trim();
    }

    public String getEmgFkCod() {
        return emgFkCod;
    }

    public void setEmgFkCod(String emgFkCod) {
        this.emgFkCod = emgFkCod == null ? null : emgFkCod.trim();
    }

    public String getPaiLvlCod() {
        return paiLvlCod;
    }

    public void setPaiLvlCod(String paiLvlCod) {
        this.paiLvlCod = paiLvlCod == null ? null : paiLvlCod.trim();
    }

    public String getFalAssCod() {
        return falAssCod;
    }

    public void setFalAssCod(String falAssCod) {
        this.falAssCod = falAssCod == null ? null : falAssCod.trim();
    }

    public String getMnsSitCod() {
        return mnsSitCod;
    }

    public void setMnsSitCod(String mnsSitCod) {
        this.mnsSitCod = mnsSitCod == null ? null : mnsSitCod.trim();
    }

    public Date getMnsLstDat() {
        return mnsLstDat;
    }

    public void setMnsLstDat(Date mnsLstDat) {
        this.mnsLstDat = mnsLstDat;
    }

    public String getAlgTypCod() {
        return algTypCod;
    }

    public void setAlgTypCod(String algTypCod) {
        this.algTypCod = algTypCod == null ? null : algTypCod.trim();
    }

    public String getSymSitDes() {
        return symSitDes;
    }

    public void setSymSitDes(String symSitDes) {
        this.symSitDes = symSitDes == null ? null : symSitDes.trim();
    }

    public String getSavMngCod() {
        return savMngCod;
    }

    public void setSavMngCod(String savMngCod) {
        this.savMngCod = savMngCod == null ? null : savMngCod.trim();
    }

    public String getSqlSeq() {
        return sqlSeq;
    }

    public void setSqlSeq(String sqlSeq) {
        this.sqlSeq = sqlSeq == null ? null : sqlSeq.trim();
    }

    public Short getOxyNbr() {
        return oxyNbr;
    }

    public void setOxyNbr(Short oxyNbr) {
        this.oxyNbr = oxyNbr;
    }

    public String getSenStuCod() {
        return senStuCod;
    }

    public void setSenStuCod(String senStuCod) {
        this.senStuCod = senStuCod == null ? null : senStuCod.trim();
    }

    public String getAlgHon() {
        return algHon;
    }

    public void setAlgHon(String algHon) {
        this.algHon = algHon == null ? null : algHon.trim();
    }

    public String getSqlDctNbr() {
        return sqlDctNbr;
    }

    public void setSqlDctNbr(String sqlDctNbr) {
        this.sqlDctNbr = sqlDctNbr == null ? null : sqlDctNbr.trim();
    }

    public String getPastHisCod() {
        return pastHisCod;
    }

    public void setPastHisCod(String pastHisCod) {
        this.pastHisCod = pastHisCod == null ? null : pastHisCod.trim();
    }

    public String getCstAgeCod() {
        return cstAgeCod;
    }

    public void setCstAgeCod(String cstAgeCod) {
        this.cstAgeCod = cstAgeCod == null ? null : cstAgeCod.trim();
    }

    public String getRefusedSurvey() {
        return refusedSurvey;
    }

    public void setRefusedSurvey(String refusedSurvey) {
        this.refusedSurvey = refusedSurvey == null ? null : refusedSurvey.trim();
    }

    public String getJbzdDes() {
        return jbzdDes;
    }

    public void setJbzdDes(String jbzdDes) {
        this.jbzdDes = jbzdDes == null ? null : jbzdDes.trim();
    }

    public String getCspgFlg() {
        return cspgFlg;
    }

    public void setCspgFlg(String cspgFlg) {
        this.cspgFlg = cspgFlg == null ? null : cspgFlg.trim();
    }

    public String getCspgCod() {
        return cspgCod;
    }

    public void setCspgCod(String cspgCod) {
        this.cspgCod = cspgCod == null ? null : cspgCod.trim();
    }

    public String getHxknFlg() {
        return hxknFlg;
    }

    public void setHxknFlg(String hxknFlg) {
        this.hxknFlg = hxknFlg == null ? null : hxknFlg.trim();
    }

    public String getHxknCod() {
        return hxknCod;
    }

    public void setHxknCod(String hxknCod) {
        this.hxknCod = hxknCod == null ? null : hxknCod.trim();
    }

    public String getHxknSubCod() {
        return hxknSubCod;
    }

    public void setHxknSubCod(String hxknSubCod) {
        this.hxknSubCod = hxknSubCod == null ? null : hxknSubCod.trim();
    }

    public String getXtCod() {
        return xtCod;
    }

    public void setXtCod(String xtCod) {
        this.xtCod = xtCod == null ? null : xtCod.trim();
    }

    public String getXtSubCod() {
        return xtSubCod;
    }

    public void setXtSubCod(String xtSubCod) {
        this.xtSubCod = xtSubCod == null ? null : xtSubCod.trim();
    }

    public String getXmCod() {
        return xmCod;
    }

    public void setXmCod(String xmCod) {
        this.xmCod = xmCod == null ? null : xmCod.trim();
    }

    public String getXmSubCod() {
        return xmSubCod;
    }

    public void setXmSubCod(String xmSubCod) {
        this.xmSubCod = xmSubCod == null ? null : xmSubCod.trim();
    }

    public String getXtFlg() {
        return xtFlg;
    }

    public void setXtFlg(String xtFlg) {
        this.xtFlg = xtFlg == null ? null : xtFlg.trim();
    }

    public String getXmFlg() {
        return xmFlg;
    }

    public void setXmFlg(String xmFlg) {
        this.xmFlg = xmFlg == null ? null : xmFlg.trim();
    }

    public String getSwChl() {
        return swChl;
    }

    public void setSwChl(String swChl) {
        this.swChl = swChl == null ? null : swChl.trim();
    }

    public String getEmgJob() {
        return emgJob;
    }

    public void setEmgJob(String emgJob) {
        this.emgJob = emgJob == null ? null : emgJob.trim();
    }

    public String getMaritalStatus() {
        return maritalStatus;
    }

    public void setMaritalStatus(String maritalStatus) {
        this.maritalStatus = maritalStatus == null ? null : maritalStatus.trim();
    }

    public String getEmgBed() {
        return emgBed;
    }

    public void setEmgBed(String emgBed) {
        this.emgBed = emgBed == null ? null : emgBed.trim();
    }

    public String getNowHis() {
        return nowHis;
    }

    public void setNowHis(String nowHis) {
        this.nowHis = nowHis == null ? null : nowHis.trim();
    }

    public String getPastHis() {
        return pastHis;
    }

    public void setPastHis(String pastHis) {
        this.pastHis = pastHis == null ? null : pastHis.trim();
    }

    public String getCaseHis() {
        return caseHis;
    }

    public void setCaseHis(String caseHis) {
        this.caseHis = caseHis == null ? null : caseHis.trim();
    }

    public String getIsFetation() {
        return isFetation;
    }

    public void setIsFetation(String isFetation) {
        this.isFetation = isFetation == null ? null : isFetation.trim();
    }

    public String getKsfz() {
        return ksfz;
    }

    public void setKsfz(String ksfz) {
        this.ksfz = ksfz == null ? null : ksfz.trim();
    }

    public String getPatid() {
        return patid;
    }

    public void setPatid(String patid) {
        this.patid = patid == null ? null : patid.trim();
    }

    public String getRyzdXy() {
        return ryzdXy;
    }

    public void setRyzdXy(String ryzdXy) {
        this.ryzdXy = ryzdXy == null ? null : ryzdXy.trim();
    }

    public String getCyzdZy() {
        return cyzdZy;
    }

    public void setCyzdZy(String cyzdZy) {
        this.cyzdZy = cyzdZy == null ? null : cyzdZy.trim();
    }

    public String getCyzdXy() {
        return cyzdXy;
    }

    public void setCyzdXy(String cyzdXy) {
        this.cyzdXy = cyzdXy == null ? null : cyzdXy.trim();
    }

    public String getObservationCode() {
        return observationCode;
    }

    public void setObservationCode(String observationCode) {
        this.observationCode = observationCode == null ? null : observationCode.trim();
    }

    public String getDeathDiagnosis() {
        return deathDiagnosis;
    }

    public void setDeathDiagnosis(String deathDiagnosis) {
        this.deathDiagnosis = deathDiagnosis == null ? null : deathDiagnosis.trim();
    }

    public String getSpecialOther() {
        return specialOther;
    }

    public void setSpecialOther(String specialOther) {
        this.specialOther = specialOther == null ? null : specialOther.trim();
    }

    public Date getDocDat() {
        return docDat;
    }

    public void setDocDat(Date docDat) {
        this.docDat = docDat;
    }

    public String getPatientTyp() {
        return patientTyp;
    }

    public void setPatientTyp(String patientTyp) {
        this.patientTyp = patientTyp == null ? null : patientTyp.trim();
    }

    public String getPregnantYdcxCod() {
        return pregnantYdcxCod;
    }

    public void setPregnantYdcxCod(String pregnantYdcxCod) {
        this.pregnantYdcxCod = pregnantYdcxCod == null ? null : pregnantYdcxCod.trim();
    }

    public String getPregnantGsplCod() {
        return pregnantGsplCod;
    }

    public void setPregnantGsplCod(String pregnantGsplCod) {
        this.pregnantGsplCod = pregnantGsplCod == null ? null : pregnantGsplCod.trim();
    }

    public Long getJzxh() {
        return jzxh;
    }

    public void setJzxh(Long jzxh) {
        this.jzxh = jzxh;
    }

    public String getJzys() {
        return jzys;
    }

    public void setJzys(String jzys) {
        this.jzys = jzys == null ? null : jzys.trim();
    }

    public String getYsxm() {
        return ysxm;
    }

    public void setYsxm(String ysxm) {
        this.ysxm = ysxm == null ? null : ysxm.trim();
    }

    public String getKsdm() {
        return ksdm;
    }

    public void setKsdm(String ksdm) {
        this.ksdm = ksdm == null ? null : ksdm.trim();
    }

    public String getCheckDoctor() {
        return checkDoctor;
    }

    public void setCheckDoctor(String checkDoctor) {
        this.checkDoctor = checkDoctor == null ? null : checkDoctor.trim();
    }

    public String getCheckNurse() {
        return checkNurse;
    }

    public void setCheckNurse(String checkNurse) {
        this.checkNurse = checkNurse == null ? null : checkNurse.trim();
    }

    public Date getCheckDocDat() {
        return checkDocDat;
    }

    public void setCheckDocDat(Date checkDocDat) {
        this.checkDocDat = checkDocDat;
    }

    public Date getCheckNurseDat() {
        return checkNurseDat;
    }

    public void setCheckNurseDat(Date checkNurseDat) {
        this.checkNurseDat = checkNurseDat;
    }

    public String getCheckDocStatus() {
        return checkDocStatus;
    }

    public void setCheckDocStatus(String checkDocStatus) {
        this.checkDocStatus = checkDocStatus == null ? null : checkDocStatus.trim();
    }

    public String getCheckNurStatus() {
        return checkNurStatus;
    }

    public void setCheckNurStatus(String checkNurStatus) {
        this.checkNurStatus = checkNurStatus == null ? null : checkNurStatus.trim();
    }

    public String getMpi() {
        return mpi;
    }

    public void setMpi(String mpi) {
        this.mpi = mpi == null ? null : mpi.trim();
    }

    public String getGhxh() {
        return ghxh;
    }

    public void setGhxh(String ghxh) {
        this.ghxh = ghxh == null ? null : ghxh.trim();
    }

    public String getRfidcode() {
        return rfidcode;
    }

    public void setRfidcode(String rfidcode) {
        this.rfidcode = rfidcode == null ? null : rfidcode.trim();
    }

    public String getVaccineHis() {
        return vaccineHis;
    }

    public void setVaccineHis(String vaccineHis) {
        this.vaccineHis = vaccineHis == null ? null : vaccineHis.trim();
    }

    public String getXfztFlg() {
        return xfztFlg;
    }

    public void setXfztFlg(String xfztFlg) {
        this.xfztFlg = xfztFlg == null ? null : xfztFlg.trim();
    }

    public String getQtsjFlg() {
        return qtsjFlg;
    }

    public void setQtsjFlg(String qtsjFlg) {
        this.qtsjFlg = qtsjFlg == null ? null : qtsjFlg.trim();
    }

    public String getAlgTypDes() {
        return algTypDes;
    }

    public void setAlgTypDes(String algTypDes) {
        this.algTypDes = algTypDes == null ? null : algTypDes.trim();
    }

    public String getBedLgs() {
        return bedLgs;
    }

    public void setBedLgs(String bedLgs) {
        this.bedLgs = bedLgs == null ? null : bedLgs.trim();
    }

    public String getFirstsqlseq() {
        return firstsqlseq;
    }

    public void setFirstsqlseq(String firstsqlseq) {
        this.firstsqlseq = firstsqlseq == null ? null : firstsqlseq.trim();
    }

    public String getCleanhzMemo() {
        return cleanhzMemo;
    }

    public void setCleanhzMemo(String cleanhzMemo) {
        this.cleanhzMemo = cleanhzMemo == null ? null : cleanhzMemo.trim();
    }

    public String getQtsjSeq() {
        return qtsjSeq;
    }

    public void setQtsjSeq(String qtsjSeq) {
        this.qtsjSeq = qtsjSeq == null ? null : qtsjSeq.trim();
    }

    public Date getMysj() {
        return mysj;
    }

    public void setMysj(Date mysj) {
        this.mysj = mysj;
    }

    public Date getMqsj() {
        return mqsj;
    }

    public void setMqsj(Date mqsj) {
        this.mqsj = mqsj;
    }

    public String getCardType() {
        return cardType;
    }

    public void setCardType(String cardType) {
        this.cardType = cardType == null ? null : cardType.trim();
    }

    public String getTriageNum() {
        return triageNum;
    }

    public void setTriageNum(String triageNum) {
        this.triageNum = triageNum == null ? null : triageNum.trim();
    }

    public String getNationality() {
        return nationality;
    }

    public void setNationality(String nationality) {
        this.nationality = nationality == null ? null : nationality.trim();
    }

    public String getNation() {
        return nation;
    }

    public void setNation(String nation) {
        this.nation = nation == null ? null : nation.trim();
    }

    public String getGzdw() {
        return gzdw;
    }

    public void setGzdw(String gzdw) {
        this.gzdw = gzdw == null ? null : gzdw.trim();
    }

    public String getGhid() {
        return ghid;
    }

    public void setGhid(String ghid) {
        this.ghid = ghid == null ? null : ghid.trim();
    }

    public String getFrCod() {
        return frCod;
    }

    public void setFrCod(String frCod) {
        this.frCod = frCod == null ? null : frCod.trim();
    }

    public Date getFrDat() {
        return frDat;
    }

    public void setFrDat(Date frDat) {
        this.frDat = frDat;
    }

    public String getFirstemgseq() {
        return firstemgseq;
    }

    public void setFirstemgseq(String firstemgseq) {
        this.firstemgseq = firstemgseq == null ? null : firstemgseq.trim();
    }

    public String getInfectedareaNam() {
        return infectedareaNam;
    }

    public void setInfectedareaNam(String infectedareaNam) {
        this.infectedareaNam = infectedareaNam == null ? null : infectedareaNam.trim();
    }

    public String getAbnScoNme() {
        return abnScoNme;
    }

    public void setAbnScoNme(String abnScoNme) {
        this.abnScoNme = abnScoNme == null ? null : abnScoNme.trim();
    }

    public String getAbnDriver() {
        return abnDriver;
    }

    public void setAbnDriver(String abnDriver) {
        this.abnDriver = abnDriver == null ? null : abnDriver.trim();
    }

    public String getAbnDoctor() {
        return abnDoctor;
    }

    public void setAbnDoctor(String abnDoctor) {
        this.abnDoctor = abnDoctor == null ? null : abnDoctor.trim();
    }

    public String getAdtA01() {
        return adtA01;
    }

    public void setAdtA01(String adtA01) {
        this.adtA01 = adtA01 == null ? null : adtA01.trim();
    }

    public String getGlsType() {
        return glsType;
    }

    public void setGlsType(String glsType) {
        this.glsType = glsType == null ? null : glsType.trim();
    }

    public String getGlsNum() {
        return glsNum;
    }

    public void setGlsNum(String glsNum) {
        this.glsNum = glsNum == null ? null : glsNum.trim();
    }

    public String getCdxx() {
        return cdxx;
    }

    public void setCdxx(String cdxx) {
        this.cdxx = cdxx == null ? null : cdxx.trim();
    }

    public String getKhlx() {
        return khlx;
    }

    public void setKhlx(String khlx) {
        this.khlx = khlx == null ? null : khlx.trim();
    }

    public String getPsryqt() {
        return psryqt;
    }

    public void setPsryqt(String psryqt) {
        this.psryqt = psryqt == null ? null : psryqt.trim();
    }

    public String getGhbz() {
        return ghbz;
    }

    public void setGhbz(String ghbz) {
        this.ghbz = ghbz == null ? null : ghbz.trim();
    }

    public String getGlsCod() {
        return glsCod;
    }

    public void setGlsCod(String glsCod) {
        this.glsCod = glsCod == null ? null : glsCod.trim();
    }

    public String getLabelPrintFlg() {
        return labelPrintFlg;
    }

    public void setLabelPrintFlg(String labelPrintFlg) {
        this.labelPrintFlg = labelPrintFlg == null ? null : labelPrintFlg.trim();
    }

    public String getXtlcflg() {
        return xtlcflg;
    }

    public void setXtlcflg(String xtlcflg) {
        this.xtlcflg = xtlcflg == null ? null : xtlcflg.trim();
    }

    public String getCzlcflg() {
        return czlcflg;
    }

    public void setCzlcflg(String czlcflg) {
        this.czlcflg = czlcflg == null ? null : czlcflg.trim();
    }
}