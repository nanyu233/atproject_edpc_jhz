...
import activetech.base.process.result.*;
...
public class ZyyemgAction {
    ...
    public String queryjsbrjz(Model model) throws Exception {
        ...
        //三墩院区预检号标识 SD
        //灵隐院区预检号标识 LY
        if ("SD".equals(HSPLP)) model.addAttribute("emgSeq", "SD" + emgseq);
        else
    ...
    public @ResponseBody SubmitResultInfo findEmgList_jzt(String emgSeq, String editbedflg) throws Exception {
        ResultInfo resultInfo = ResultUtil.createSuccess(Config.MESSAGE, 906, null);
        Map<String, String> params = new HashMap<>(60);
...
//		map.put("bedplacecodList",bedplacecodList);
    ...
    public SubmitResultInfo exportfzgllbSubmit(HspemginfQueryDto hspemginfQueryDto,
        ...
        // 导出文件的前缀
        final String filePrefix = "fzgllb";
        // -1表示关闭自动刷新，手动控制写磁盘的时机，其它数据表示多少数据在内存保存，超过的则写入磁盘
        final int flushRows = 100;
        // 定义导出数据的title
        ...
        final String hb = "就诊时间分布统计表";
        String gb = "查询范围: ";
        String startTime = DateUtil.formatDateByFormat(hspemginfQueryDto.getStartdate(), "yyyy-MM-dd");
        String endTime = DateUtil.formatDateByFormat(hspemginfQueryDto.getEnddate(), "yyyy-MM-dd");
    ...
    public SubmitResultInfo exportqjsSubmit(HspemginfQueryDto hspemginfQueryDto,
        ...
        for (HspemginfCustom hspemginfCustom : list) {
            ...
            if ("1".equals(flg) || !StringUtils.isNotNullAndEmptyByTrim(flg)) hspemginfCustom.setCheckNurStatus("未查对");
            else if ("2".equals(flg)) hspemginfCustom.setCheckNurStatus("已查对");
        }
        ...
        for (HspemginfCustom hspemginfCustom : list)
            if ("1".equals(hspemginfCustom.getIsBed())) {
                ...
                } else hspemginfCustom.setIsBed("");

        //腕带打印赋值
        for (HspemginfCustom hspemginfCustom : list)
            if ("1".equals(hspemginfCustom.getLabelPrintFlg())) hspemginfCustom.setLabelPrintFlg("已打印");
            else
        ...
        String filePath = systemConfigService.findAppoptionByOptkey("XNML_PATH").getOptvalue();
        // 导出文件的前缀
        final String filePrefix = "qjsgllb";
        // -1表示关闭自动刷新，手动控制写磁盘的时机，其它数据表示多少数据在内存保存，超过的则写入磁盘
        final int flushRows = 100;
        // 定义导出数据的title
        List<String> fieldNames = new ArrayList<>();
    ...
    public DataGridResultInfo findHspScoListBySzm(HspemginfQueryDto hspemginfQueryDto) {
        ...
        if (hspemginfQueryDto.getAcronym() == null) hspemginfQueryDto.setAcronym("");
    ...
    public DataGridResultInfo findAbnDriverList(HspemginfQueryDto hspemginfQueryDto) {
        String name;
        if (hspemginfQueryDto.getHspemginfCustom() == null) name = "";
        else
    ...
    public DataGridResultInfo findEmgInfForWeek_result(HspemginfQueryDto hspemginfQueryDto,
        ...
        List<HspemginfCustom> findEmgInfByCad = new ArrayList<>();
        if (hspemginfQueryDto.getStartdate() == null) {
            Calendar calendar = Calendar.getInstance();
            calendar.set(Calendar.DAY_OF_YEAR, calendar.get(Calendar.DAY_OF_YEAR) - 6);
            Date enddate = calendar.getTime();
    ...
    public String toSubdivision(Model model, String emgSeq) throws Exception {
        ...
        //三墩院区预检号标识 SD
        //灵隐院区预检号标识 LY
        if ("SD".equals(HSPLP)) model.addAttribute("newEmgSeq", "SD" + emgseq);
        else
