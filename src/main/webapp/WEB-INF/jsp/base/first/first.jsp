...
<link rel="shortcut icon" type="image/x-icon"
	href="${baseurl}images/hzszyyhems/favicon.gif">
<%@ include file="/WEB-INF/jsp/base/common_css.jsp"%>
<link rel="stylesheet" type="text/css"
	href="${baseurl}css/hzszyyhems/first.css?v=${versionDay}">
<link rel="stylesheet" type="text/css"
	href="${baseurl}lib/at-font/style.css?v=${versionDay}">
...
<script type="text/javascript">
    ...
    function addTab(subtitle, url, icon, reloadtype) {
      //如果指定标题的tab不存则添加一个新的
      var _isCloseFlg = true;
      if (url.indexOf('toLeftWrap_HIS') != -1) {
        _isCloseFlg = false
      }
    ...
    $(function () { //预加载方法
      ...
      /*
		if (!publicFun.getItem('addrInfoList')) {
    ...
    function buildDangerSocket() {
      publicFun.httpServer({ url: '${baseurl}icuer/isMachineJudgment.do' }, {}, function (res) {
    ...
    function timingBox() {
      if (isCloseBox) {
        window.top.createmodalwindow("危急值结果确认", 600, 300, '${baseurl}hzszyyemg/confirmCritical.do', '', false);
        isCloseBox = false;
      }
    }
    //退出系统方法
    ...
    function queryDangerMsg() {
      parent.opentabwindow('危急值消息查询', '${baseurl}icuer/toErInfo.do?moduleid=' + 'c408dbf33d304f4e826761380d8a044f');
    }
    ...
    var firstVm = avalon.define({
      ...
      firstLineClick: function (current) {
        if ($(current).attr('hideUrl')) {
          ...
          // 	window.open('${baseurl}report/querybigscreen.do');
          // } else {
      ...
      secondLineClick: function (current) {
        ...
        if ($(current).attr('hideSubUrl') === 'report/querybigscreen.do') {
          ...
          window.open('${baseurl}zyyqjs/qjsScreen.do');
        } else {
          addTab(current.title, current.rel, icon, reloadType);
        }
        $('.second-menu').hide();
...
<body class="easyui-layout first" scroll="no">
	<div border="false" split="false" region="north" class="top-bg">
		<div class="first-bg">
			&nbsp;&nbsp;<img src="images/hzszyyhems/logo.png" /><!-- <img src="images/hzszyyhems/libanglogo.png" /> --><span
				class="first-l"> ${hospitalPlatformName} </span> <span
        ...
        </a> --> <a href="javascript:queryOutime()"> <span
					class="isShowMessage">候诊提醒 <img
						src="images/hems/first/tip_icon.png" class="msgIcon"
						id="msg_alert" /> <span class="bg-theme" id="notifyMsg">无</span>
				</span>
			</a> <img class="iconfirst iconnone"
				src="images/hems/first/personal2.png"><span class="showUser">${activeUser.usrname}(${activeUser.usrno}),欢迎您！</span>
				<a title="回到首页" href="javascript:toFirst()" id="#" class="index-btn">【回到首页】</a>
				<a title="密码设置" ref="modifypwd" href="javascript:updatepwd()"
				id="modifypwd" class="repwd-btn">【密码设置】</a> <a title="退出"
				id="loginOut" href="javascript:logout()" class="restart-btn">【退出】</a>
			</span> <a title="急诊大屏" class="bigScreen"
				href="${baseurl}report/querybigscreen.do" target="_Blank"><img
		...
		<div class="four-level-menu" ms-controller="menuWrap">
			<ul class="first-menu">
				...
				<li class="first-menu-li" ms-repeat="menuList"
					ms-mouseover="setNextMenuDisplay($index, 'show')"
					ms-mouseout="setNextMenuDisplay($index, 'hide')"><i
					ms-class="my-icon nav-icon at-fa-{{el.icon}}"></i> <a
					class="first-link" ms-click="firstLineClick(this)"
					ms-attr-reloadtype="el.reloadType" ms-attr-ref="el.menuid"
					ms-attr-rel="{{el.mainurl}}?moduleid={{el.menuid}}"
					ms-attr-icon="el.icon" ms-attr-title="el.menuname"
					ms-attr-hideUrl="el.mainurl">{{el.menuname}}</a>
					<ul class="second-menu" ms-if="el.menus && !el.mainurl">
						<iframe frameborder="0" scrolling="no" class="iframeNo"></iframe>
						<li ms-repeat-item="el.menus"><a href="javascript:;"
							class="second-link" ms-attr-reloadtype="item.reloadType"
							ms-attr-ref="item.menuid"
							ms-attr-rel="{{item.url}}?moduleid={{item.menuid}}"
							ms-attr-icon="item.icon" ms-attr-title="item.menuname"
							ms-click="secondLineClick(this)" ms-mouseover="isHoverMenu(this)"
