...
<body class="editdevice">
  <form class="form" id="deviceform" action="${baseurl}device/updatehds.do" method="post">
    <table>
      <colgroup>
        <col width="23%" />
        <col />
      ...
      <tbody>
        <tr>
          <th>
            <input type="hidden" name="hspDevSysCustom.id" value="${hspDevSysCustom.id }"><i class="red">*</i> 设备类型：</th>
          <td>
            <select id="devtypCod" name="hspDevSysCustom.devtypCod" class="li-input-s">
              <c:forEach items="${dictlist}" var="value">
                <option value="${value.infocode}" <c:if test="${value.infocode eq hspDevSysCustom.devtypCod }">
                  selected="selected"
        ...
        <tr>
          ...
          <td>
            <input type="text" id="devno" name="hspDevSysCustom.devno" maxlength="32" value="${hspDevSysCustom.devno }" />
          </td>
        </tr>
        <tr>
          <th><i class="red">*</i> 绑定客户端IP：</th>
          <td>
            <input type="text" id="ipaddr" name="hspDevSysCustom.ipaddr" maxlength="32" value="${hspDevSysCustom.ipaddr }" />
          </td>
        </tr>
        <tr>
          <th>备注：</th>
          <td>
            <textarea class="div-textarea divarea" id="memo" name="hspDevSysCustom.memo" maxlength="100">${hspDevSysCustom.memo }</textarea>
          </td>
        </tr>
        <tr>
          <td colspan="2" class="last_td">
            <div class="center grp_btn">
              <a class="easyui-linkbutton" iconCls="icon-ok" id="submitbtn" onclick="updateDevice()">确定</a>
  ...
  <script type="text/javascript">
    ...
    function updateDevice() {
      if (validateForm()) {
        jquerySubByFId('deviceform', updatedevice_callback, null, "json");
      }
    }
    ...
    //新增的回调函数
    function updatedevice_callback(data) {
      message_alert(data);
