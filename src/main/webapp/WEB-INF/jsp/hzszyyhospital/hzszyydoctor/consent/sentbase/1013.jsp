	...
	<div class="messages m_t10">
		<div style="border: 0px">
			<table class="tbody">
				...
				<tr>
					<td colspan="5">
						<label id="id_7_1" class="labtitle">
						一、医师告知事项
						</label><br/>
						<label id="id_7_2">
						...
						<label id="id_7_5">
						2.导致延误疾病诊断、对病情变化不能明确或及时了解，影响进一步诊治，导致不良后果。
						</label><br/>
						<label id="id_7_6">
						3.将使患者原有疾病的治疗中断，病情有可能会出现反复甚至有可能加重或进行性加重，将会使以后的治疗变得更加困难，甚至无法治愈或丧失最佳治疗时机。
						</label><br/>
						<label id="id_7_7">
						4.有可能会导致患者出现各种感染或原有感染加重、伤口延迟愈合、疼痛等各种症状加重或时间的延长。
						</label><br/>
						<label id="id_7_8">
						5.有可能会导致患者某个或多个器官功能减退、部分或全部丧失，导致出现功能障碍、诱发其他疾病、出血、休克等。
						</label><br/>
						<label id="id_7_9">
				...
				<tr>
					<td colspan="5">
						...
						<div style="text-indent: 2em">
							<label id="id_11">医务人员已经向患方详尽解释了此医疗措施在医学上的依据和必要性。患方已经获得充足时间对患者的状况以及拒绝此医疗措施所产生的不良后果及风险有了充分了解。</label>
						</div>
						<div style="text-indent: 2em">
							<label id="id_12">患方经慎重考虑，自愿承担拒绝此医疗措施的风险和后果，拒绝此医疗措施所产生的不良后果与医院及医务人员无关。</label>
						</div>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="3">
						<label id="id_16">注：签名人须为年满18周岁且具有完全民事行为能力。</label>
					</td>
					<td colspan="5" align="right">
						<label id="id_17">医师：</label>
...
<script type="text/javascript">
	function getText(){
		...
		var lab7_8 = document.getElementById("id_7_8").innerText;
		var lab7_9 = document.getElementById("id_7_9").innerText;
		var lab7_10 = document.getElementById("id_7_10").innerText;	
		...
		text +=	"<div style=\"text-indent: 0em\"><strong>"+lab7_1+"</strong></div>";
		...
		text +=	"<div style=\"text-indent: 0em\">"+lab7_7+"</div>";
		text +=	"<div style=\"text-indent: 0em\">"+lab7_8+"</div>";
		text +=	"<div style=\"text-indent: 0em\">"+lab7_9+"</div>";
		text +=	"<div style=\"text-indent: 0em\">"+lab7_10+"<strong>"+value7+"</strong></div>";
		text +=	"<table border='0' width='100%' cellpadding='0' cellspacing='0'><tr height='27'><td>"+lab8+value8+" "+lab9+value9+"</td></tr></table>";
		...
		text +=	"<table border='0' width='100%' cellpadding='0' cellspacing='0'><tr height='27'><td>"+lab16+"</td><td align='right'>"+lab17+value13+"&nbsp&nbsp"+dzqm+"</td></tr></table>";
		text += "</div>";
		$("#content").val(text);
		return true;
	}
	...
	function getparam(){
	    ...
	    var texthtml=document.getElementById("s1013").innerHTML;
		texthtml="<div id='s1013'>"+texthtml+"</div>";
	 	document.getElementById("texthtml").value=texthtml;
	 	return true;
	}
	$(function(){
		$('#id_title2').html('患者拒绝或放弃诊疗告知术书')
	})
</script>
</div>
