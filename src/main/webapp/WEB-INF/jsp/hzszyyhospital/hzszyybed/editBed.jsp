...
<body class="adddevice">
  <form class="form" id="bedform" action="${baseurl}bedinf/editbed.do" method="post">
    <table>
      <colgroup>
        <col width="23%" />
        <col />
      ...
      <tbody>
        <tr>
          <th><input type="hidden" name="hspBedInfCustom.bedid" value="${hspBedInf.bedid }"><i class="red">*</i> 位置：</th>
          <td>
            <select id="bedPlace" name="hspBedInfCustom.bedPlace" class="li-input-s">
              <c:forEach items="${stateList}" var="value">
                <option value="${value.infocode}" <c:if test="${value.infocode eq hspBedInf.bedPlace }">
                  selected="selected"
        ...
        <tr>
          ...
          <td>
            <input type="text" id="bedNum" name="hspBedInfCustom.bedNum" maxlength="32" value="${hspBedInf.bedNum }" />
          </td>
        </tr>
        <tr>
          <th><i class="red">*</i> 排序：</th>
          <td>
            <input type="text" id="showorder" name="hspBedInfCustom.showorder" maxlength="32" value="${hspBedInf.showorder }"
